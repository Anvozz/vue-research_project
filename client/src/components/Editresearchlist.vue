    ...
    <b-container fluid>
          ...
          <b-col lg="4" class="my-1">
            ...
            >
              <b-form-checkbox-group v-model="filterOn" class="mt-1">
                ...
                <b-form-checkbox value="research_head"
                  >ชื่อเรื่อง</b-form-checkbox
                >
      ...
      >
        ...
        <template #cell(actions)="row">
          <b-button
            size="sm"
            @click="editresearch(row.item, row.index)"
            class="mr-1"
...
export default {
  ...
  methods: {
    ...
    onresearchpush() {
      Researchapi.GetuserResearch({ userid: this.userid }).then((res) => {
        if (res.data) {
          this.items = res.data;
          this.totalRows = this.items.length;
        }
    ...
    editresearch(item, index) {
      this.$router.push(
        `/editresearch/edit/${this.userid}/${item.research_id}`
      );
    },
    deleteresearch(item, index) {
      this.notify({ text: "กำลังลบงานวิจัย", type: "warn" });
      Researchapi.DeleteResearch({
        userid: this.userid,
        index: item.research_id,
      }).then((res) => {
        if (res.data) {
